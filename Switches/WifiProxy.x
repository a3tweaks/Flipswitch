...
+ (void)stateDidChange
{
	...
	if (sharedSwitch) {
		[[FSSwitchPanel sharedPanel] stateDidChangeForSwitchIdentifier:@"com.a3tweaks.switch.wifi-proxy"];
	} else if (IsEnabled()) {
		sharedSwitch = [[self alloc] _init];
		[[FSSwitchPanel sharedPanel] registerDataSource:sharedSwitch forSwitchIdentifier:@"com.a3tweaks.switch.wifi-proxy"];
	}
}
